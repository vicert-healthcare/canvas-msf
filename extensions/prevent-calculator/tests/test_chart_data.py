...
from prevent_calculator.lib import chart_data
from prevent_calculator.lib.chart_data import (
    ChartPrefill,
    ChartValue,
    _matches_any,
...
from prevent_calculator.lib.loinc import LOINC_TOTAL_CHOLESTEROL
...
def test_resolve_bmi_handles_fhir_imported_lb_weight_without_units() -> None:
    """Regression test for an implausible BMI ≈ 1.5 seen on a test instance.
...
def test_resolve_smoking_picks_up_snomed_coded_observation() -> None:
    """A smoking-status observation that carries a SNOMED coding (no
    LOINC) was previously missed. We now look it up via the SNOMED
