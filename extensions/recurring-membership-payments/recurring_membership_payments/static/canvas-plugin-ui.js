(function() {
  ...
  /* Shared with the two search components, which build option rows and chips from
     strings that came off the page as text. */
  ...
  /* The remote search engine shared by canvas-combobox and canvas-multi-select.
     It owns the query, the debounce timer, the minimum length gate, the request
     token and the phase, and it touches no DOM at all. That is what makes the two
     components behave identically, and it is what lets this be reasoned about
     without a browser in the way none of the per component copies could be.

     The host contract is three methods. emitSearch fires the search event,
     renderPhase draws the status row, applyResults swaps the option set. The
     engine calls nothing else and names no component, so neither component knows
     the other exists and a third consumer costs one adapter. JavaScript has no
     compiled interface to lean on, so the contract is a documented set of required
     methods plus the validation below, rather than a guarantee it cannot give. */

  var SEARCH_HOST_CONTRACT = ['emitSearch', 'renderPhase', 'applyResults', 'clearResults'];

  function SearchController(host) {
    for (var i = 0; i < SEARCH_HOST_CONTRACT.length; i++) {
      var method = SEARCH_HOST_CONTRACT[i];
      if (!host || typeof host[method] !== 'function') {
        throw new TypeError('SearchController host must implement ' + method + '()');
      }
    }
    this._host = host;
    this._debounce = 200;
  ...
  SearchController.prototype.configure = function(opts) {
    if (!opts) return;
    if (opts.debounce != null && !isNaN(opts.debounce)) this._debounce = Math.max(0, Number(opts.debounce));
    if (opts.minChars != null && !isNaN(opts.minChars)) this._minChars = Math.max(0, Number(opts.minChars));
    if (opts.timeout != null && !isNaN(opts.timeout)) this._timeout = Math.max(0, Number(opts.timeout));
  };

  SearchController.prototype._clearTimers = function() {
    if (this._timer) { clearTimeout(this._timer); this._timer = null; }
    if (this._timeoutTimer) { clearTimeout(this._timeoutTimer); this._timeoutTimer = null; }
  };

  /* renderPhase fires on every transition rather than only on a change of value,
     because two consecutive searches both landing on ready still need the status
     row redrawn against the newer query. */
  SearchController.prototype._setPhase = function(phase) {
    this._phase = phase;
    this._host.renderPhase(phase, this._query);
  };

  /* Every keystroke invalidates whatever is in flight by bumping the token, so a
     response that arrives after this point can no longer be applied. That single
     line is the whole of the out of order guarantee, and it lives here rather
     than in each component because the Canvas consumers that hand rolled it
     mostly forgot it. */
  SearchController.prototype.queryChanged = function(text) {
    var query = text == null ? '' : String(text);
    if (query === this._query) return;
    this._query = query;
    this._token++;
    this._clearTimers();
    /* The rows on screen answered the previous query, so they go the moment the
       query changes rather than lingering underneath the new one. Keeping them
       would leave answers to a question nobody is asking any more, still
       selectable, which on a drug search is a wrong pick rather than a stale
       pixel. A brief empty panel is the honest state and the cheaper mistake. */
    this._host.clearResults();
    var trimmed = query.trim();
    if (trimmed.length === 0 || trimmed.length < this._minChars) {
      this._settled = this._token;
      this._setPhase('idle');
      return;
    }
    this._setPhase('pending');
    var self = this;
    this._timer = setTimeout(function() { self._fire(); }, this._debounce);
  };

  SearchController.prototype._fire = function() {
    var self = this;
    var token = this._token;
    this._timer = null;
    this._setPhase('searching');
    if (this._timeout > 0) {
      this._timeoutTimer = setTimeout(function() {
        if (self._token !== token || self._settled >= token) return;
        self._settled = token;
        self._failure = 'The search did not respond.';
        self._setPhase('failed');
      }, this._timeout);
    }
    this._host.emitSearch(this._query, token);
  };

  /* A token that is not the newest, a token already settled, and a token for a
     search that was never emitted are all refused. So a consumer cannot write
     stale results even deliberately, and the phase can only be moved by the one
     event it is actually waiting on. */
  SearchController.prototype._accepts = function(token) {
    return token === this._token && this._settled < token && this._phase === 'searching';
  };

  SearchController.prototype.submitResults = function(token, rows) {
    if (!this._accepts(token)) return false;
    this._settled = token;
    this._clearTimers();
    this._failure = '';
    this._host.applyResults(Array.isArray(rows) ? rows : []);
    this._setPhase('ready');
    return true;
  };

  SearchController.prototype.submitFailure = function(token, message) {
    if (!this._accepts(token)) return false;
    this._settled = token;
    this._clearTimers();
    this._failure = message == null ? '' : String(message);
  ...
  Object.defineProperty(SearchController.prototype, 'phase', { get: function() { return this._phase; } });
  Object.defineProperty(SearchController.prototype, 'query', { get: function() { return this._query; } });
  Object.defineProperty(SearchController.prototype, 'token', { get: function() { return this._token; } });
  Object.defineProperty(SearchController.prototype, 'failure', { get: function() { return this._failure; } });
  ...
  class CanvasCombobox extends HTMLElement {
    static get observedAttributes() {
      return ['label', 'placeholder', 'value', 'disabled', 'required', 'error', 'name', 'empty-state', 'creatable', 'creatable-label', 'loading', 'loading-label',
        'remote', 'debounce', 'min-chars', 'search-timeout', 'search-hint', 'searching-label', 'search-error-label'];
    }
    ...
    constructor() {
      ...
      this._search = new SearchController(this._searchHost());
    ...
    attributeChangedCallback(name) {
      if (name === 'debounce' || name === 'min-chars' || name === 'search-timeout') {
        this._configureSearch();
        return;
      }
      if (name === 'creatable') {
    ...
    _render() {
      ...
      var optionsHtml = this._optionsHtml();
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          /* The remote search status row. It is a sibling of the empty row rather
             than a reuse of it, because "nothing was asked yet", "asking now" and
             "asked and found nothing" are three different sentences and only the
             last one is an empty state. */
          .search-status { padding: .78571429rem 1.14285714rem; font-size: 1rem; color: rgba(0, 0, 0, 0.4); display: none; }
          .search-status.visible { display: flex; align-items: center; gap: .5em; }
          .search-status.busy::before {
            content: ''; width: 1em; height: 1em; flex-shrink: 0;
            border: 2px solid rgba(34, 36, 38, 0.15); border-top-color: #767676;
            border-radius: 50%; animation: canvas-search-spin .6s linear infinite;
          }
          @media (prefers-reduced-motion: reduce) { .search-status.busy::before { animation: none; } }
          @keyframes canvas-search-spin { to { transform: rotate(360deg); } }
        ...
        <div class="combobox">
          ...
          <ul class="menu" id="${this._listboxId}" role="listbox"${label ? ' aria-labelledby="' + this._labelId + '"' : ''}>
            ...
            <li class="search-status" role="status" aria-live="polite"></li>
          </ul>
        </div>
        ${error ? '<span class="error-text" id="' + this._errorId + '">' + error + '</span>' : ''}
      `;
      /* The status row is part of the render rather than something painted onto it
         afterwards, so any full rebuild puts it back in the state the controller
         is actually in instead of blanking it. */
      if (this._isRemote()) this._renderPhaseRow(this._search.phase, this._search.query);
    }
    ...
    _bindEvents() {
      ...
      input.addEventListener('input', function() {
        if (self.hasAttribute('loading')) return;
        if (!self._open) self._openMenu();
        /* The two modes are exclusive. Local filtering would otherwise run a
           substring test over a label the server already matched on something
           else entirely, and throw away the result it was asked for. */
        if (self._isRemote()) self._search.queryChanged(input.value);
        else self._filter(input.value);
      });
    ...
    /* ---- Remote search, driven by the shared SearchController ---- */

    _isRemote() { return this.hasAttribute('remote'); }

    _configureSearch() {
      this._search.configure({
        debounce: this.getAttribute('debounce'),
        minChars: this.getAttribute('min-chars'),
        timeout: this.getAttribute('search-timeout')
      });
    }
    ...
    _searchHost() {
      var self = this;
      return {
        emitSearch: function(query, token) {
          self.dispatchEvent(new CustomEvent('search', {
            detail: { query: query, token: token }, bubbles: true, composed: true
          }));
        },
        renderPhase: function(phase, query) { self._renderPhaseRow(phase, query); },
        applyResults: function(rows) { self._applyResults(rows); },
        clearResults: function() { self._clearResults(); }
      };
    }

    /* The result set is replaced wholesale and the trigger is never touched, which
       is what keeps the typed query, the caret and the focus exactly where the user
       left them while results land underneath. */
    _applyResults(rows) {
      this._options = [];
      for (var i = 0; i < rows.length; i++) {
        var r = rows[i] || {};
        var val = r.value != null ? String(r.value) : '';
        var label = r.label != null ? String(r.label) : val;
        this._options.push({
          value: val,
          label: label,
          html: r.html != null ? String(r.html) : escapeHtml(label),
          disabled: !!r.disabled,
          domId: cuid('cb-opt')
        });
      }
    ...
    _clearResults() {
      this._options = [];
      if (this.shadowRoot.querySelector('.menu')) this._renderMenu();
    }
    ...
    _renderMenu() {
      ...
      if (!menu) return;
      var emptyState = this.getAttribute('empty-state');
      menu.innerHTML = this._optionsHtml()
        + '<li class="empty"><slot name="empty">' + (emptyState != null ? escapeHtml(emptyState) : 'No results') + '</slot></li>'
        + '<li class="search-status" role="status" aria-live="polite"></li>';
      this._highlighted = -1;

      this._renderPhaseRow(this._search.phase, this._search.query);
    }

    _renderPhaseRow(phase, query) {
      var row = this.shadowRoot.querySelector('.search-status');
      if (!row) return;
      var text = '';
      /* pending and searching read the same to a person. The debounce window is
         still the machine working on what was typed, and leaving it blank would
         paint the No results row for 200ms on the way to every answer. */
      var busy = phase === 'pending' || phase === 'searching';
      if (phase === 'idle') text = this.getAttribute('search-hint') || 'Type to search';
      else if (busy) text = this.getAttribute('searching-label') || 'Searching';
      else if (phase === 'failed') text = this._search.failure || this.getAttribute('search-error-label') || 'Search failed';
      row.textContent = text;
      row.classList.toggle('visible', text !== '');
      row.classList.toggle('busy', busy);
      /* Whenever the status row owns the panel the no results row must not also
         claim it, or a search nobody has run yet reads as one that found nothing. */
      var empty = this.shadowRoot.querySelector('.empty');
      if (empty && text !== '') empty.classList.remove('visible');
      else this._checkEmpty();
    }

    /* Public API. Both return false when the response is stale, so a caller can
       see that its result was dropped rather than assume it landed. */
    setResults(token, rows) { return this._search.submitResults(token, rows); }
    setSearchFailed(token, message) { return this._search.submitFailure(token, message); }
    resetSearch() { this._search.reset(); }
    get searchPhase() { return this._search.phase; }
    ...
    _render() {
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .text {
            flex: 1;
            /* Holds the row's height when there is nothing to show. With no selection and
               no placeholder this span renders empty, and an empty flex child establishes
               no line box, so the trigger collapsed to its padding and rendered about half
               the height of the input beside it. Matches the line-height on .dropdown. */
            min-height: 1.21428571em;
            white-space: nowrap;
            overflow: hidden;
            text-overflow: ellipsis;
            color: var(--color-text, rgba(0, 0, 0, 0.87));
  ...
  */
  ...
  class CanvasMultiSelect extends HTMLElement {
    ...
    _readOptions() {
      ...
      for (var i = 0; i < opts.length; i++) {
        ...
        if (opt.hasAttribute('selected') && this._selected.indexOf(val) === -1) {
          this._labels[val] = opt.getAttribute('label') || opt.textContent.trim();
    ...
    _renderChips() {
      ...
      /* The label is read from the remembered selection rather than looked up in
         the live option list. A remote search replaces that list on every query, so
         a lookup would delete the chip for anything the newest result set happens
         not to contain, which is the defect that made plugins keep dead options in
         the DOM to hold their own chips up. */
      for (var i = 0; i < this._selected.length; i++) {
        var val = this._selected[i];
        var label = escapeHtml(this._labelFor(val));
        var chip = document.createElement('span');
        chip.className = 'chip';
        chip.dataset.value = val;
        chip.innerHTML = label + '<button type="button" class="chip-dismiss" aria-label="Remove ' + label + '"><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M1.5 1.5l7 7M8.5 1.5l-7 7" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></button>';
    ...
    /* The one place that clears what was typed. Both modes end up with an empty
       box, and in remote mode the controller is told, so the panel returns to its
       hint row rather than leaving the component believing a query is still live
       that the user can no longer see. */
    _clearQuery() {
      var input = this.shadowRoot.querySelector('.input');
      if (input) input.value = '';
      if (this._isRemote()) this._search.queryChanged('');
    ...
    _bindEvents() {
      ...
      menu.addEventListener('click', function(e) {
        ...
        /* Picking clears the box in both modes. The chip is now what records the
           choice, so leaving the term behind states it twice and puts the next
           search behind a manual delete. */
        self._clearQuery();
        input.focus();
      });
    }
    ...
    _labelFor(val) {
      if (this._labels && this._labels[val] != null) return this._labels[val];
      var opt = this._options.find(function(o) { return o.value === val; });
    ...
    _rememberLabel(val) {
      var opt = this._options.find(function(o) { return o.value === val; });
      if (opt) this._labels[val] = opt.label;
      else if (this._labels[val] == null) this._labels[val] = val;
    }
